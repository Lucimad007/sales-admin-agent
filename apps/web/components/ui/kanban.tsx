...
function Kanban<T>({ value, onValueChange, getItemValue, children, className, onMove }: KanbanRootProps<T>) {
  ...
  const originContainer = React.useRef<string | null>(null);
  ...
  const handleDragStart = React.useCallback(
    (event: DragStartEvent) => {
      ...
      originContainer.current = isColumn(event.active.id)
        ? (event.active.id as string)
        : (findContainer(event.active.id) ?? null);
    },
  ...
  const handleDragOver = React.useCallback(
    (event: DragOverEvent) => {
      ...
      if (!movedItem) return;
      overItems.splice(overIndex < 0 ? overItems.length : overIndex, 0, movedItem);
  ...
  const handleDragEnd = React.useCallback(
    (event: DragEndEvent) => {
      ...
      if (onMove && !isColumn(active.id)) {
        const fromContainer = originContainer.current ?? findContainer(active.id);
        const overContainer = findContainer(over.id);
        originContainer.current = null;

        if (fromContainer && overContainer && fromContainer !== overContainer) {
          const activeIndex = (columns[fromContainer] ?? []).findIndex((item: T) => getItemValue(item) === active.id);
...
function KanbanOverlay({ children, className }: KanbanOverlayProps) {
  ...
  React.useLayoutEffect(() => {
    ...
    const element = document.querySelector(
      `[data-slot="kanban-${isColumn(activeId) ? "column" : "item"}"][data-value="${globalThis.CSS.escape(String(activeId))}"]`,
    );
    if (element) {
  ...
  return (
    <DragOverlay adjustScale={false} dropAnimation={dropAnimationConfig} zIndex={80}>
      <div
        data-slot="kanban-overlay"
        data-dragging={true}
        style={style}
        className={cn("pointer-events-none origin-top-left", className, activeId ? "!cursor-grabbing" : "")}
      >
        {content}
      </div>
    </DragOverlay>
